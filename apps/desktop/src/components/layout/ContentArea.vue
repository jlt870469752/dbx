...
<template>
  <div class="production-session-shell flex flex-col min-h-0" :class="[resultOnly ? 'h-full' : 'flex-1', { 'production-session-shell--active': activeProductionContext.active }]">
    ...
    <template v-else-if="activeTab.mode === 'mq'">
      <div class="flex-1 min-h-0">
        <MqAdminConsole ref="mqAdminConsoleRef" :key="activeTab.id" :connection-id="activeTab.connectionId" :initial-tenant="activeTab.mqTenant" :initial-tab="activeTab.mqInitialTab" :read-only="connectionIsEffectivelyReadOnly(activeConnection)" />
      </div>
    </template>
